...
test <- data.frame(List = as.numeric(formattedOccData_BNM$occDetdata$L),
                   source = as.factor("BNM"))

test2 <- data.frame(List = as.numeric(formattedOccData_BMS$occDetdata$L),
                   source = as.factor("UKBMS"))

test3 <- rbind(test, test2)
require(ggplot2)
ggplot(test3, aes(List, stat(density), colour = source)) +
  geom_freqpoly(aes(linetype=source, color=source)) + xlab("ListLength") + ylab("Density") #+ theme(legend.position = "none") +
...
# Merge the formated BMS, BNM and collection. Each contains two list and I will merge the two lists of the two datasets separately. 
occDetdata_merged_30March2020 <- rbind(formattedOccData_BMS$occDetdata,
                                   formattedOccData_BNM$occDetdata,
                                   formattedOccData_colls$occDetdata)
spp_vis_merged_30March2020 <- rbind(formattedOccData_BMS$spp_vis,
                                formattedOccData_BNM$spp_vis, 
                                formattedOccData_colls$spp_vis)

# Export
#write.csv(occDetdata_merged_30March2020, '../data/formattedData/occDetdata_merged_2020-03-30.csv', row.names=FALSE)
#write.csv(spp_vis_merged_30March2020, '../data/formattedData/spp_vis_merged_2020-03-30.csv', row.names=FALSE)
...
sum((formattedOccData_BMS$occDetdata$L)) # 4491312
length(unique((formattedOccData_BMS$occDetdata$site))) # 2577
sum((formattedOccData_BNM$occDetdata$L)) # 1186044
...
require(reshape2)
ypsBNM <- rowSums(acast(occDetdata_merged_24March2020, site ~ TP, length, value.var = 'L') > 0)
...
nrow(recordsUsed) # 5727158 records from 2749 sites
...
occDetdata_merged_24March2020_double <- occDetdata_merged_24March2020
occDetdata_merged_24March2020_double$visit <- as.factor(occDetdata_merged_24March2020_double$visit)

# Remove source column
recordsUsed2 <- recordsUsed[,c(1:4,6)]

# Merge with occDetdata_merged_24March2020_double
recordsUsed2 <- merge(recordsUsed2, occDetdata_merged_24March2020_double, by="visit", all.x = TRUE, all.y = FALSE)
...
levels(recordsUsed3$L)[levels(recordsUsed3$L)=="1"] <- "BMS"
levels(recordsUsed3$L)[levels(recordsUsed3$L)=="2"] <- "BNM"
levels(recordsUsed3$L)[levels(recordsUsed3$L)=="10"] <- "NHCs"
...
LLsymmary <- visitsSummary2(occDetdata_merged_24March2020)
LLsymmary
